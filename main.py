...
def search_for_term_on_google(*args: tuple):
    ...
    try:
        for _ in search(search_term,  # что искать
                        ...
                        num=3,  # количество результатов на странице
                        start=0,  # индекс первого извлекаемого результата
                        stop=3,  # индекс последнего извлекаемого результата (я хочу, чтобы открывался первый результат)
...
def get_weather_forecast(*args: tuple):
    ...
    try:
        ...
        weather_api_key = os.getenv("MY_API_KEY")
...
commands = {
    ...
    ("video", "youtube", "watch", "видео", "найди на ютубе", "найди на ютюбе"): search_for_video_on_youtube,
    ("wikipedia", "definition", "about", "определение", "википедия",
     "найди что означает"): search_for_definition_on_wikipedia,
    ("translate", "interpretation", "translation", "перевод", "перевести", "переведи"): get_translation,
    ("language", "язык", "смени язык"): change_language,
...
if __name__ == "__main__":
    ...
    person = OwnerPerson()
    person.name = "Anton"
    person.home_city = "Санкт-Петербург"
